...
public class BeepPrintExample {
	...
	public static void main(String[] args) {
		 ...
		 * "띵"이라는 문자열을 5번 출력된다. 하지만, 우리가원하는 것은 "띵"이라는
		 * 문자열이 출력되면서 비프음도 같이 출력되는것을 원한다.
		 * 이것이 바로 싱글 스레드의 단점이다. 순차적이라는 것이다.
		 * 이것을 멀티스레드로 바꿔가는 것이 스레드 장에서 우리가 해야 할 일이다.
		 * 비프음을 5번 반복해서 소리나게 함.
		 * Toolkit은 AWT(Abstract Window Toolkit)추상클래스를 상속받은 또다른 추상클래스이다.
		 * 그 중 GUI관련된 메서드들이 있다. getDefaultToolkit()는 기본적인 메서드를 구현해놓은 메서드이다.
		 */
		
		Toolkit toolkit = Toolkit.getDefaultToolkit();
		for(int i=0; i<5; i++) {
			toolkit.beep();//비프음
			//정적 메서드 sleep()은 실행하는 스레드를 0.5초가 멈추는기능
