...
public class SeedExample {
	...
	public static void main(String[] args) {
		//Math.random()과 Random클래스의 가장 큰 차이점은 종자값(seed)을
		//설정할수 있는것에 차이가 있다.
		//Random에서 종자값(seed)을 설정하면, 항상 같은 난수를 같은 순서대로 반환한다는 것
		...
		System.out.println("= rand2(항상같은값 반환) =");
		
		for(int i=0; i<5;i++) {
			System.out.println(i+" : "+rand2.nextInt()%10);
		}
		
		System.out.println("= rand3(항상 같은값 반환) =");
		for(int i=0; i<5;i++) {
