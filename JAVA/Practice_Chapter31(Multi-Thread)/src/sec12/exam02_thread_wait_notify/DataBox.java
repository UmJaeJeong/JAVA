...
public class DataBox {
	...
	public synchronized String getData() {
		...
		this.data = null;
		//다른 스레드(생산자 스레드)를 꺠운다.(즉 실행대기상태(Runnalbe)로 만든다)
		notify();
		return returnValue;
	}
	
	public synchronized void setData(String data){
		//ConsumerThread가 데이터를 읽지 없다면...
		if(this.data != null) {
			try {
				wait();
			} catch (InterruptedException e) {
		...
		notify();			//다른 스레드(ConsumerThread)를 꺠운다.
	}
	
	/*
	 * 결과론적으로 wait()와 notify()를 이용하여 번갈아 가면서 데이터
	 * 즉 공유객체에 접근하기 위해서는 synchronized란 동기화 키워드가
	 * 필히 들어가야 예외도 발생하지 아니하고, 데이터의 정확성을 보장
	 * 한다. 즉, 다시 말해 객체 잠금이 일어나야 할ㄴ다는 것이다.
	 */
